 ...
 * NOTE: Only SSL/TLS connections are supported. STARTTLS is not implemented.
 */
...
export class Pop3Client {
  ...
  constructor(config: ConnectionConfig, password: string) {
    ...
    // Only SSL is supported - STARTTLS not implemented
    if (config.encryption !== 'ssl') {
      throw new Error('POP3 only supports SSL encryption. STARTTLS is not implemented.');
    }
    this.secure = true;
  }
